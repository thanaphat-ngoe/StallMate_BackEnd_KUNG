...
router.put("/:seller_id/orders/:order_id/completed", isLoggedIn, async (req, res) => {
	try {
		const sellerId = req.params.seller_id;
		...
		if (!order) {
			return res.status(404).json({ error: "Order not found for the given seller" });
		}
...
router.get("/:seller_id/orders/completed", isLoggedIn, async (req, res) => {
	try {
		...
		console.log("Fetching completed orders for seller ID:", sellerId);
		...
		const ordersList = completedOrders.map((order) => ({
			orderId: order._id,
...
router.get("/:seller_id/orders/pending", isLoggedIn, async (req, res) => {
	try {
		...
		console.log("Fetching pending orders for seller ID:", sellerId);
