...
const upload = multer({ storage: multerS3({ s3: s3Client, bucket: process.env.AWS_BUCKET_NAME, acl: 'public-read', metadata: (req, file, cb) => {
...
router.get('/:seller_id/menu', isLoggedIn, async (req, res) => {
    try {
        ...
        /*const queryString = new URLSearchParams({
            ...
            categories: JSON.stringify(categorizedMenu) // If it's an object or array
        }).toString();

        res.redirect(`http://localhost:5173/resnmenu?${queryString}`);*/
...
router.get('/:seller_id/menu/category/:category', isLoggedIn, async (req, res) => {
  	try {
      	...
      	if (!menu) {
          	return res.status(404).json({ message: 'No menu found for this seller' });
      	}

    	const categoryItems = menu.items.filter(item => item.category === requestedCategory).map(item => ({
    		_id: item._id,
            name: item.name,
            name_en: item.name_en,
            description: item.description,
...
router.post('/:seller_id/menu/:item_id', isLoggedIn, upload.single('image'), async (req, res) => {
  	...
  	if (category && !Object.values(CATEGORIES).includes(category)) {
      	return res.status(400).json({ error: `Invalid category. Must be one of: ${Object.values(CATEGORIES).join(', ')}` });
  	}
