  	...
  	async (request, accessToken, refreshToken, profile, done) => {
    	try {
     		...
     		if (existingUser) {
        		...
        		const newUser = new User({ username: profile.displayName, email: profile.emails[0].value, googleID: profile.id });
	...
	async (request, accessToken, refreshToken, profile, done) => {
		try {
			...
			if (existingUser) {
				...
				const newCustomer = new Customer({ username: profile.displayName, email: profile.emails[0].value, googleID: profile.id });
...
passport.deserializeUser(async ({ id, userType }, done) => {
	try {
		...
		const foundUser = userType === 'StallOwner' ? await User.findById(id) : await Customer.findById(id);
